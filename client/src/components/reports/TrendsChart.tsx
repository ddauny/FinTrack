...
export const TrendsChart: React.FC<TrendsChartProps> = ({ data, isDark, loading }) => {
  ...
  const option = useMemo(() => ({
    ...
    xAxis: { 
      type: 'category', 
      boundaryGap: false, 
      data: data.map(r => formatDateDMY(new Date(r.period + '-01'))), 
    ...
    series: [
      { 
        ...
        smooth: true, 
        showSymbol: false,
        data: data.map(r => Math.abs(r.total_expense)), 
      ...
      { 
        ...
        smooth: true, 
        showSymbol: false,
        data: data.map(r => r.total_income), 
  ...
  return (
    <div className="bg-white dark:bg-[#111111] p-5 rounded-lg border border-slate-200 dark:border-[#1f1f1f] h-full">
      <h3 className="text-lg font-bold text-slate-800 dark:text-[#f0f0f0] mb-4">Income vs Expense Trend</h3>
      {loading ? (
