...
export function TopNav() {
  ...
  const link = (to: string, label: string) => (
    <Link
      to={to}
      className={`px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${pathname === to
          ? 'bg-stone-100 text-stone-900 dark:bg-stone-700/50 dark:text-white shadow-sm'
          : 'text-stone-500 hover:text-stone-900 hover:bg-stone-50 dark:text-stone-400 dark:hover:text-white dark:hover:bg-stone-800/50'
        }`}
    >
      {label}
    </Link>
  ...
  const mobileLink = (to: string, label: string) => (
    <Link
      ...
      className={`block px-4 py-3 text-sm font-medium rounded-lg mx-2 transition-colors ${pathname === to
          ? 'bg-stone-100 text-stone-900 dark:bg-stone-700/50 dark:text-white'
          : 'text-stone-500 hover:text-stone-900 hover:bg-stone-50 dark:text-stone-400 dark:hover:text-white dark:hover:bg-stone-800/50'
        }`}
    >
      {label}
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-stone-200 dark:border-stone-800 bg-white/80 dark:bg-stone-900/80 backdrop-blur-md">
      <div className="mx-auto max-w-7xl flex items-center justify-between p-3">
        <div className="flex items-center gap-2">
          <Link to="/" className="font-bold text-xl tracking-tight text-stone-900 dark:text-white flex items-center gap-2">
            FinTrack
          </Link>
        </div>
          ...
          {token && (<>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="p-2 rounded-lg text-stone-500 hover:text-stone-900 hover:bg-stone-100 dark:text-stone-400 dark:hover:text-white dark:hover:bg-stone-800 transition-all"
        ...
        <div className="md:hidden">
          <button
            ...
            className="p-2 rounded-md text-stone-700 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-700 focus:outline-none focus:ring-2 focus:ring-stone-500"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-stone-800 border-t dark:border-t-stone-700">
          <div className="px-2 pt-2 pb-3 space-y-1">
            {token && (<>
              ...
              <button
                ...
                className="flex items-center w-full px-4 py-3 text-sm font-medium text-stone-700 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800/50 rounded-lg mx-2 transition-colors"
              >
                <span className="mr-2">
                  {resolved === 'light' ? (
